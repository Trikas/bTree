...
class BTreeService
{
     ...
     */
    public static function getInfoAboutNode($node, $bTree)
    {
        self::$level++;
        self::$path[] = $node->id;
        ...
        if ($node->parent_id) {
            if ($node->position == 1) {
                $parentNode = $bTree->where('id', $node->parent_id)->where('left', $node->id)->first();
                self::getInfoAboutNode($parentNode, $bTree);
            } elseif ($node->position == 2) {
                $parentNode = $bTree->where('id', $node->parent_id)->where('right', $node->id)->first();
                self::getInfoAboutNode($parentNode, $bTree);
            }
        }
